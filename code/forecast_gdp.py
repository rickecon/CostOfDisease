...
def extend_gdp_series(
    ...
    gdp_series = pd.Series(data=gdp_df["real_gdp"].values, index=gdp_df["year"])
    ...
    x = np.linspace(-5, 5, convergence_years)  # Years before and after the transition
    ...
    if forecast_years > convergence_years:
        growth_rates = np.concatenate(
            [growth_rates, [long_term_growth] * (forecast_years - convergence_years)]
        )
    ...
    if save_path:
        extended_series.to_csv(save_path, header=["real_gdp"], index_label="year")

    # --- Optional Plot ---
    if plot:
        plt.figure(figsize=(10, 5))
        plt.plot(extended_series, label="Real GDP (Historical + Forecast)", marker="o")
        plt.axvline(
            gdp_series.index[-1], color="gray", linestyle="--", label="Forecast Start"
        )
